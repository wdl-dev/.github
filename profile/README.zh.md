...
随着项目逐步开放，这里还会放更多一方仓库。公开 WDL site [wdl.dev](https://wdl.dev/) 是一个部署到 WDL hosted preview 上的普通 Worker：用 WDL 把一方应用部署到 WDL。
...
WDL 开发者的工作分布在 platform、CLI、docs、examples 和一方 Workers 之间。一方应用应通过和普通 tenant workload 相同的 CLI 和 control plane 部署。

## Dogfooding

WDL 通过运行真实应用来开发。公开 site 是 hosted preview 上的一方 Worker。WDL Chat 是更大的内部 dogfooding workload：一个运行在 WDL 上的 AI-assisted Worker 开发应用，可以生成 WDL Worker 代码、部署到 test namespaces，并调用 preview URL。
...
文档放在各自仓库中。Docker images 发布为 [`getwdl/wdl-workerd`](https://hub.docker.com/r/getwdl/wdl-workerd) 和 [`getwdl/wdl-rust`](https://hub.docker.com/r/getwdl/wdl-rust)。联系：<hi@wdl.dev>。
